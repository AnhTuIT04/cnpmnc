...
export const CriteriaList = ({ criteriaList = [], pagination }: CriteriaListProps) => {
  ...
  // Ensure criteriaList is always an array
  const safeCriteriaList = Array.isArray(criteriaList) ? criteriaList : [];
  ...
  const filtered = safeCriteriaList.filter((assess) => categoryFilter === "All" || assess.category === categoryFilter);

  // Use data directly from API (already paginated)
  // Note: If you want server-side category filtering, move it to API params
      ...
      {pagination && pagination.totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 mt-4">
          ...
          <span className="text-sm font-medium text-gray-700 px-4">
            Trang {pagination.currentPage} / {pagination.totalPages} (Tổng: {pagination.totalElements} tiêu chí)
          </span>
          <button
            onClick={() => handleChangePage(Math.min(pagination.totalPages, pageFromUrl + 1))}
            disabled={pageFromUrl >= pagination.totalPages}
            className="px-4 py-2 border border-gray-300 rounded-md disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-colors"
          >
            Sau
          </button>
